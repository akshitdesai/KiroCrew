...
describe('OfficeViewer', () => {
  ...
  it('names the download action without naming a button label it cannot match', async () => {
    // span 0db4b73ded01: the line used to end "or download the original" while
    // the one button beside it reads "Download", which a blind reader took for
    // two different controls. The copy names the ACTION so it cannot disagree.
    stubPreview({ error: 'boom' }, false, 500)
    renderWithQuery(<OfficeViewer filePath="/home/user/docs/quarterly-report.docx" />)
    await waitFor(() => {
      expect(screen.getByTestId('office-preview-error')).toBeInTheDocument()
    })
    expect(screen.getByText(/or download it instead/i)).toBeInTheDocument()
    expect(screen.queryByText(/download the original/i)).toBeNull()
  })
