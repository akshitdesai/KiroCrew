...
describe('FileBrowserRail content results', () => {
  ...
  it('says visibly, once, that document hits open from the start', async () => {
    ...
    expect(screen.getByTestId('file-grep-doc-note')).toHaveTextContent(/open the file from the start/)
  })
  ...
  it('names the documents the budget did not reach', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('file-grep-status').textContent)
        .toContain('at least 4 documents not searched'))
  })
  ...
  it('reads correctly for a single skipped document', async () => {
    ...
    mount()
    await searchContents('needle')
    // "at least 1 documents" is deliberately accepted. The count is a FLOOR, so
    ...
    // wording exists to end. No ICU plural is introduced: an awkward singular
    // beats four Russian plural forms nobody here can verify.
    await waitFor(() =>
      expect(screen.getByTestId('file-grep-status').textContent)
        .toContain('at least 1 documents not searched'))
  })
