 ...
 *  app"); the reader needs the task and the destination -- the Apps page --
 *  so the two codes a user can act on are said that way; anything else is
    ...
    return (
      <span className="inline-flex items-center gap-1.5">
        <button
          ...
          disabled={again}
          title={!card.hireable ? unavailableReason(card, t) : undefined}
...
function TemplateDetail({ card, pending, error, labelOf, onClose, onHire, onChat }: { card: HireTemplateCard | null; pending: boolean; error: string; labelOf: (id: string) => string; onClose: () => void; onHire: (c: HireTemplateCard) => void; onChat: (c: HireTemplateCard) => void }) {
  const { t } = useTranslation()
    ...
    <Dialog open={!!card} onOpenChange={(o) => { if (!o) onClose() }}>
      {card && (
        <DialogContent maxWidth={640} className="p-0" data-testid="hire-template-detail">
                  ...
                  <ul className="list-none m-0 p-0 mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5" data-testid="hire-detail-caps">
                    {card.capabilities.map((c) => (
                      <li key={`${c.kind}:${c.name}`} className="flex items-center gap-2 text-[12.5px] min-w-0">
                        ...
                        <span className="text-[10px] uppercase tracking-wide text-muted ml-auto shrink-0">{t(CAPABILITY_KIND_KEY[c.kind])}</span>
                      </li>
                    ))}
                  </ul>
                )}
          ...
          <div className="shrink-0 flex flex-col gap-2 px-7 py-4 border-t border-border bg-card">
            {/* A failed hire from THIS dialog is said here, where the user is
                looking: the page-level notice sits behind the open modal and
                "Hiring…" would otherwise simply revert with no message. No
                hand-off (askAgent={false}): the dialog is the unsaved state
                -- the card the user chose -- and Ask Agent navigates away
                from it; the retry is the same button. */}
            {error && <ErrorNotice message={error} variant="inline" askAgent={false} testId="hire-detail-error" />}
...
export default function HireGalleryPage() {
  ...
  return (
    <div className="flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden" data-testid="hire-gallery">
      ...
      <TemplateDetail card={detail} pending={pendingId !== null && pendingId === detail?.id} error={detail ? hireError : ''} labelOf={labelOf} onClose={() => setDetailId(null)} onHire={(c) => hire.mutate(c)} onChat={openChat} />
    </div>
  )
}
