...
describe('HireGalleryPage', () => {
  ...
  it('lists cards from all sources with role, duty, three tags and the right button', async () => {
    ...
    expect(within(off).getByTestId('hire-button')).toHaveAttribute('title', 'ghost hasn’t finished setting up this agent. Open Apps, turn ghost off and on again, then come back.')
  ...
  it('a refused hire from the detail dialog is said inside the dialog, where the user is looking', async () => {
    ...
    // In the dialog footer -- the page-level notice would sit behind the modal.
    const notice = await within(detail).findByTestId('hire-detail-error')
    expect(notice).toHaveTextContent('Oncall pack is disabled')
    expect(screen.queryByTestId('hire-error')).toBeNull()
    // No hand-off from inside the dialog: the retry is the same button.
    expect(within(notice).queryByRole('button', { name: /ask the agent/i })).toBeNull()
    expect(within(detail).getByTestId('hire-button')).toBeInTheDocument()
  })
