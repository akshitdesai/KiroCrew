...
read through the repository's validated-read chokepoint, which refuses a symlink
or hard link swapped in at the aside name rather than dereferencing it. The
destination is published with an atomic no-clobber `os.link` of privately staged
bytes, falling back to an `O_CREAT | O_EXCL` create on filesystems without hard
links, so a settings file recreated at the
