...
file (`source == "app"`) -- or one whose dedup DROPPED an app's file of that name (`AgentInfo.shadowed_sources` carries the sources the listing gave up in favour of the kept entry; a name an app and an earlier-sorting package both ship is ambiguous, not proof the app stopped shipping it, so the app row and its memory stay -- pinned: `test_a_name_an_app_and_a_package_both_ship_keeps_the_app_row`): a local or package agent that happens to share the name
