...
class TestGitStatus:
    ...
    async def test_symlink_git_marker_to_real_repo_is_followed_by_git(
        ...
        project.mkdir()
        marker = project / ".git"
        marker.symlink_to(repo / ".git", target_is_directory=True)
    ...
    async def test_symlink_git_marker_to_missing_target_returns_repo_false(
        ...
        project.mkdir()
        (project / ".git").symlink_to(tmp_path / "absent-target", target_is_directory=True)
    ...
    @pytest.mark.asyncio
    async def test_lock_suffix_head_ref_returns_repo_false(self, repo, mock_sel):
