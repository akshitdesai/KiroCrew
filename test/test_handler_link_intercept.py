...
# ── Bare `sessions` keyword fall-through in a linked thread (#10865) ──
...
class TestSessionsKeywordFallThrough:
    ...
    picker is unreachable in a linked thread (issue #10865)."""
    ...
    async def test_handle_message_reaches_sessions_command_when_linked(self):
        ...
        try:
            ...
            # End to end: the linked DM no longer swallows the keyword — the
            # native session picker path runs and the slot gets no user row.
