...
@pytest.mark.asyncio
async def test_a_credential_shaped_member_is_dropped_not_rewritten(tmp_path, mock_sel):
    """A member is handed straight back to /api/file-office-media, so a REWRITTEN
    one names an entry the archive does not contain and breaks a real picture.
    One that redaction would change is dropped to ``None`` instead -- the state a
    picture the backend cannot serve already uses, so the frontend has a
    placeholder for it -- and the name reaches neither the JSON nor the image
    URL."""
    secret = "AKIAIOSFODNN7EXAMPLE"
    member = f"word/media/{secret}.png"
    f = tmp_path / "credpic.docx"
    write_docx(
        str(f),
        docx_image_para("rId1") + docx_para("body"),
        images={"rId1": f"media/{secret}.png"},
        media={member: PNG_1X1},
    )
    with patch("kiro_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-office-preview?path={f}&format=blocks")
    ...
    assert json.loads(raw)["blocks"][0] == {"type": "image", "member": None}
