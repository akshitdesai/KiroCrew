...
def _read_app_bytes(
    root: Path, root_fd: int | None, rel: str, cap: int, *, what: str
) -> bytes | None:
    """The bytes of an app-tree file named RELATIVE to the app root, or ``None``
    for a missing, refused or oversized one.

    With *root_fd* (POSIX) every component of *rel* is walked ``O_NOFOLLOW``
    relative to the root descriptor the caller pinned right after validation
    ...
    files read as the template's. Without descriptor support (Windows) the
    path-pinned read keeps the planted-final-link refusal, as everywhere else.
    """
    if root_fd is None:
        return _read_bytes_pinned(root / rel, cap, what=what)
    try:
...
def _verify_card_content(
    app: str, card: CrewTemplate, root: Path, root_fd: int | None, *, signature_required: bool
) -> dict[str, bytes]:
...
def resolve_store_template(app: str, agent_path: str) -> StoreTemplate:
    ...
    # descriptor instead (``pinned_fs.read_bytes_at``). POSIX only; the path
    # flow elsewhere keeps its planted-final-link refusal.
...
def _resolve_from_root(
    ...
    *,
    ...
    verified = _verify_card_content(app, card, root, root_fd, signature_required=signature_required)
    ...
    spec_bytes = (
        ...
        if "agent" in verified
            ...
            root, root_fd, agent_path, TEMPLATE_SPEC_MAX_BYTES, what="template agent file"
        )
    )
