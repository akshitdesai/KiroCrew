...
_BLOCK_STRUCTURAL_KEYS = frozenset({"type", "level", "ordered", "index", "bold", "italic"})
...
def _redact_block(block: object) -> object:
    """Redact one block, every string by default, with two carve-outs.
    ...
    **``member`` is dropped, never rewritten.** It is a ZIP member identifier the
    frontend hands straight back to /api/file-office-media, so a rewritten one
    names an entry the archive does not contain -- a redacted member would turn a
    legitimate picture into a broken request. A member that redaction WOULD change
    becomes ``None`` instead, which is the state a picture the backend cannot
    serve already uses, so the frontend renders its existing placeholder and the
    name never reaches the JSON or the image URL.
    """
    ...
    for key, value in block.items():
        if key in _BLOCK_STRUCTURAL_KEYS:
            ...
            out[key] = None if isinstance(value, str) and redact(value) != value else value
        else:
