...
#: ahead of it, and resident memory stays one line plus the capped hits.
_GREP_RG_QUEUE_LINES = 256
...
def _grep_rg_executable() -> str | None:
    ...
    The world-writable test is POSIX-only. Windows synthesizes ``st_mode`` --
    any writable file reports ``0o666``, so ``S_IWOTH`` is set on every ordinary
    install and consulting it there would refuse ripgrep platform-wide while
    saying nothing about the actual ACL. Windows is screened from the real
    security descriptor instead, through the same
    ``check_provider_path_component_windows`` the provider-CLI check uses: an
    unreadable descriptor, a non-local volume, a third-account owner or any
    untrusted writer refuses the binary. Fail CLOSED -- an unverifiable SID or an
    ACL that cannot be read takes the python engine rather than spawning
    something the check could not clear.
    ...
    if sys.platform == "win32":
        # The mode bits carry no information here, so the ACL is the only
        # evidence. Not strict: the gateway user's OWN install is legitimate,
        # exactly as the POSIX branch accepts a file it owns.
        from kiro_crew.github_runner import check_provider_path_component_windows

        me_sid = platform_compat.current_user_sid() or ""
        if not me_sid:
            logger.warning("file_grep: gateway SID unverifiable; refusing rg")
            return None
        try:
            check_provider_path_component_windows(
                resolved, label="rg", me_sid=me_sid, strict=False
            )
        except ValueError as exc:
            logger.warning("file_grep: refusing rg at %s: %s", resolved, exc)
            return None
    elif mode & _stat_mod.S_IWOTH:
...
def _grep_rg(root: str, query: str, deadline: float) -> tuple[list[dict], bool] | None:
    ...
    try:
        wrapped, cleanup = wrap_argv(_grep_rg_argv(root, query, executable))
        proc = popen_limited(
        ...
        lines: "queue.Queue[str | None]" = queue.Queue(maxsize=_GREP_RG_QUEUE_LINES)
        stdout = proc.stdout
        ...
        while True:
            ...
            if remaining <= 0:
                ...
                break
            try:
                record_line = lines.get(timeout=remaining)
            except queue.Empty:
                # Nothing arrived inside the budget: rg is still traversing, and
                # the answer is what we have, marked short. This is the case the
                # inline read could not see at all.
                capped = True
                break
...
def _grep_doc_segments(data: bytes, path: str, ext: str) -> tuple[tuple[str, str], ...]:
    ...
    ``Sheet1 row 12`` for a worksheet row, and ``doc`` for a Word document, whose
